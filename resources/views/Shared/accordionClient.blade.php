    ...
    <div id="6" class="card mb-4" style="display: none">
        ...
        <div class="card-body">
            ...
            <form autocomplete="off" action="{{url('/loans')}}{{$loan!=null?'/'.$loan->id:''}}"method="post" style="font-size:14px">
                ...
                <div class="justify-content-end d-flex">
                    <button {{auth()->check()?'':( $client?->quality_holder_id==2||$client?->quality_holder_id==3?'disabled':'')}}   type="submit" id="btnGuardar" class="btn btn-success">
                        {{$loan==null?'Guardar':'Actualizar'}}
                    </button>
                </div>
            </form>
        </div>
